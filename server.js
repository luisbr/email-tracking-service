...
function renderHululLogo(className = "brand-logo") {
  ...
  const openedFilter = className === "logo" ? `<script>(()=>{if(new URLSearchParams(location.search).get('opened')!=='1')return;const priorFetch=window.fetch.bind(window);window.fetch=(resource,...rest)=>{const url=new URL(resource instanceof Request?resource.url:String(resource),location.origin);if(url.pathname==='/api/admin/links')url.searchParams.set('opened','1');return priorFetch(url.pathname+url.search,...rest)};})();</script>` : "";
  return `<img class="${className}" src="/brand/hulul-logo.png" alt="HULUL">${trackingFilter}${openedFilter}`;
}
...
function renderPlatformPage() {
  ...
  <section class="view" id="campaigns"><div class="form-actions" role="tablist" aria-label="Vistas de campañas" style="margin-bottom:20px"><button type="button" class="primary campaign-tab" data-campaign-pane="send">Enviar</button><button type="button" class="campaign-tab" data-campaign-pane="new">Nueva</button><button type="button" class="campaign-tab" data-campaign-pane="history">Historial</button></div><article class="panel campaign-pane" id="campaignNewPane" hidden><h2>Nueva campaña</h2><p>Una campaña reúne audiencia, contenido y perfil de envío.</p><form id="campaignForm" class="form-grid"><label>Cliente<select name="clientId" class="clientSelect" required></select></label><label>Nombre<input name="name" required placeholder="Ej. Lanzamiento octubre"></label><label>Asunto<input name="subject" placeholder="Asunto del correo"></label><label>Audiencia<select name="audienceId" id="audienceSelect"><option value="">Seleccionar después</option></select></label><label>Versión de template<select name="templateVersionId" id="templateVersionSelect"><option value="">Seleccionar después</option></select></label><label>Perfil SES<select name="sesAccountId" id="sesSelect"><option value="">Seleccionar después</option></select></label><div class="full form-actions"><button class="primary">Crear campaña</button></div></form></article><article class="panel campaign-pane" id="campaignSendPane"><h2>Enviar campaña</h2><p>Envía una prueba primero. Una vez aprobada, podrás iniciar el envío a su audiencia.</p><div class="form-grid"><label>Campaña<select id="deliveryCampaign"><option value="">Selecciona una campaña</option></select></label><label>Correo de prueba<input id="deliveryTestEmail" type="email" placeholder="nombre@dominio.com"></label><div class="full form-actions"><button type="button" id="sendTestButton">Enviar prueba</button><button type="button" id="approveCampaignButton">Aprobar campaña</button><button type="button" class="primary" id="sendCampaignButton">Enviar a audiencia</button><span class="notice" id="deliveryResult"></span></div></div></article><article class="panel campaign-pane" id="campaignHistoryPane" hidden><h2>Historial de campañas</h2><p>Consulta el estado de cada envío y abre su tracking individual.</p><table class="table"><thead><tr><th>Campaña</th><th>Cliente</th><th>Estado</th><th>Audiencia</th><th></th></tr></thead><tbody id="campaignRows"></tbody></table></article></section>
  ...
  <script>const deliveryCampaignSelect=document.querySelector('#deliveryCampaign'),testAction=document.querySelector('#sendTestButton'),approveAction=document.querySelector('#approveCampaignButton'),sendAction=document.querySelector('#sendCampaignButton');const updateDeliveryActions=()=>{const status=deliveryCampaignSelect.selectedOptions[0]?.dataset.status;testAction.hidden=!status||!['draft','test_ready'].includes(status);approveAction.hidden=status!=='test_sent';sendAction.hidden=status!=='approved'};testAction.hidden=approveAction.hidden=sendAction.hidden=true;fetch('/api/platform/campaigns').then(response=>response.json()).then(data=>{deliveryCampaignSelect.innerHTML='<option value="">Selecciona una campaña</option>'+data.rows.map(campaign=>'<option value="'+campaign.id+'" data-status="'+campaign.status+'">'+campaign.name+' · '+campaign.status+'</option>').join('');updateDeliveryActions()}).catch(()=>{});deliveryCampaignSelect.addEventListener('change',updateDeliveryActions);</script>
  <script>window.addEventListener('DOMContentLoaded',()=>{const panes={send:document.querySelector('#campaignSendPane'),new:document.querySelector('#campaignNewPane'),history:document.querySelector('#campaignHistoryPane')},tabs=document.querySelectorAll('.campaign-tab');const showCampaignPane=name=>{Object.entries(panes).forEach(([key,pane])=>{pane.hidden=key!==name});tabs.forEach(tab=>tab.classList.toggle('primary',tab.dataset.campaignPane===name))};tabs.forEach(tab=>tab.addEventListener('click',()=>showCampaignPane(tab.dataset.campaignPane)));const rows=document.querySelector('#campaignRows');new MutationObserver(()=>rows.querySelectorAll('tr').forEach(row=>{if(row.querySelector('.tracking-campaign-link')||row.cells.length<4)return;const name=row.cells[0].textContent.trim(),cell=document.createElement('td'),link=document.createElement('a');link.className='tracking-campaign-link';link.href='/admin/tracking?campaign='+encodeURIComponent(name);link.textContent='Ver tracking';cell.append(link);row.append(cell)})).observe(rows,{childList:true});});</script>
  <script>window.addEventListener('DOMContentLoaded',()=>{const history=document.querySelector('#campaignHistoryPane'),progress=document.createElement('section');progress.innerHTML='<h2 style="margin-top:28px">Progreso de envíos</h2><div id="sendJobHistory" class="empty">Cargando progreso...</div>';history.append(progress);const refreshJobs=()=>fetch('/api/platform/send-jobs').then(response=>response.json()).then(data=>{const target=document.querySelector('#sendJobHistory');target.innerHTML=data.rows.length?'<table class="table"><thead><tr><th>Campaña</th><th>Estado</th><th>Avance</th><th>Enviados</th><th>Pendientes</th><th>Fallidos</th></tr></thead><tbody>'+data.rows.map(job=>'<tr><td>'+job.campaignName+'</td><td><span class="badge">'+job.status+'</span></td><td>'+job.processed+' / '+job.total+' ('+job.percentage+'%)</td><td>'+job.sent+'</td><td>'+job.pending+'</td><td>'+job.failed+'</td></tr>').join('')+'</tbody></table>':'<strong>Sin envíos creados</strong>'}).catch(()=>{document.querySelector('#sendJobHistory').textContent='No se pudo cargar el progreso.'});refreshJobs();setInterval(refreshJobs,5000)});</script>
...
async function handlePlatformRequest(request, response, requestUrl, session) {
  try {
    ...
    if (request.method === "GET" && pathName === "/api/platform/send-jobs") {
      const [rows] = await pool.execute(`SELECT j.id, j.campaign_id AS campaignId, c.name AS campaignName, j.status, j.total, j.processed, j.sent, j.failed, j.pending, j.started_at AS startedAt, j.finished_at AS finishedAt, j.last_activity_at AS lastActivity FROM send_jobs j JOIN campaigns c ON c.id = j.campaign_id WHERE c.account_id = ?${scopedClientId ? " AND c.client_id = ?" : ""} ORDER BY j.id DESC LIMIT 30`, scopedClientId ? [account.id, scopedClientId] : [account.id]);
      return sendJson(response, 200, { ok: true, rows: rows.map((job) => ({ ...job, percentage: job.total ? Number(((job.processed / job.total) * 100).toFixed(2)) : 0 })) });
    }
